...
class ImageCaptioningModel(tf.keras.Model):
    ...
    def call(self, images, captions, training=False):
        ...
        mask = (captions != 0)
        predictions = self.decoder(captions, encoder_output, training=training, mask=mask)
...
# Build the model with a dummy input to create the variables
dummy_img_input = tf.random.uniform((1, 224, 224, 3))  # Dummy image input
dummy_caption_input = tf.random.uniform((1, MAX_LENGTH), maxval=VOCABULARY_SIZE, dtype=tf.int32)  # Dummy caption input
caption_model(cnn_model(dummy_img_input), dummy_caption_input)
...
def generate_caption(image):
    ...
    caption_tokens = caption_model.predict(preprocessed_image)
